...
def train():
    ...
    with tf.Graph().as_default():
        ...
        # input
        train_input = CNNTrainInput(FLAGS.data_dir,
                                    crop_size=crop_size)
        if FLAGS.use_fp16:
            FP = tf.float16
        else:
        ...
        image_slices = tf.slice(images, 
            [0,0,0,0], 
            [int(batch_size), crop_size[0], crop_size[1], 1],
            name='central_slice')
        ...
        with tf.Session() as sess:
            ...
            for step in range(FLAGS.max_steps):
                batch_images, batch_labels = train_input.next_micro_batch(batch_size=batch_size)
                if FLAGS.shuffle:
